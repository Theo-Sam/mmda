...
import { Building2, Eye, EyeOff, Loader2, Shield, Moon, Sun, UserCog, User, Briefcase, Users, FileText, Globe, Building, UserCheck, UserCircle, Lock as LockIcon } from 'lucide-react';
  ...
  const roles = [
    {
      key: 'Super Admin',
      label: 'Super Admin',
      icon: <Shield className="w-6 h-6" />, // lucide-react
      demo: true,
    },
    {
      key: 'Regional Admin',
      label: 'Regional Admin',
      icon: <User className="w-6 h-6" />, // lucide-react
      demo: true,
    },
    {
      key: 'MMDA Admin',
      label: 'MMDA Admin',
      icon: <Building2 className="w-6 h-6" />, // lucide-react
      demo: true,
    },
    {
      key: 'Finance Officer',
      label: 'Finance Officer',
      icon: <Briefcase className="w-6 h-6" />, // lucide-react
      demo: true,
    },
    {
      key: 'Auditor',
      label: 'Auditor',
      icon: <UserCheck className="w-6 h-6" />, // lucide-react
      demo: true,
    },
    {
      key: 'Monitoring Body',
      label: 'Monitoring Body',
      icon: <Globe className="w-6 h-6" />, // lucide-react
      demo: true,
    },
    {
      key: 'Business Registration Officer',
      label: 'Business Registration Officer',
      icon: <UserCircle className="w-6 h-6" />, // lucide-react
      demo: true,
    },
    {
      key: 'Collector',
      label: 'Collector',
      icon: <FileText className="w-6 h-6" />, // lucide-react
      demo: true,
    },
  ];
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 overflow-hidden">
      ...
      <div className="max-w-md w-full space-y-8 z-10">
        ...
        {!selectedRole ? (
          <div className="bg-white/60 dark:bg-gray-800/60 rounded-3xl shadow-2xl p-8 backdrop-blur-lg border border-white/40 dark:border-gray-700/50 transition-all duration-300 hover:shadow-3xl animate-fade-in-slow">
            {/* Role selection cards */}
            <div className="mb-6">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2 text-center">Select your role to login</h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 animate-fade-in">
                {roles.map(role => (
                  <button
                    key={role.key}
                    type="button"
                    className={`flex flex-col items-center justify-center p-4 rounded-xl border transition-all duration-200 shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 text-gray-700 dark:text-gray-200 bg-white/90 dark:bg-gray-700/90 hover:bg-blue-100 dark:hover:bg-blue-900/40 border-gray-200 dark:border-gray-700 scale-100 hover:scale-105 active:scale-95 group relative animate-pop-in`}
                    style={{ boxShadow: '0 2px 12px 0 rgba(60,60,120,0.07)' }}
                    onClick={() => {
                  ...
                  >
                    {role.icon}
                    <span className="mt-2 text-xs font-semibold text-center whitespace-pre-line leading-tight">{role.label}</span>
                  </button>
                ))}
              </div>
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-2 text-center">Only the roles shown above can login to the system.</p>
            </div>
          </div>
        ) : (
          <div className={`bg-white/70 dark:bg-gray-800/70 rounded-3xl shadow-2xl p-8 backdrop-blur-2xl border border-white/40 dark:border-gray-700/50 transition-all duration-300 hover:shadow-3xl animate-slide-in-up ${shakeError ? 'animate-shake' : ''}`}>
            ...
            <button
              type="button"
              className="mb-4 flex items-center text-sm text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300 font-medium transition-all duration-200 px-3 py-1 rounded-lg bg-blue-50 dark:bg-blue-900/30 shadow-sm border border-blue-100 dark:border-blue-800"
              onClick={() => {
            ...
            <div className="flex flex-col items-center mb-6">
              <div className="flex items-center space-x-2 px-4 py-2 rounded-xl bg-gradient-to-r from-blue-100 to-indigo-100 dark:from-blue-900/40 dark:to-indigo-900/40 shadow border border-blue-200 dark:border-blue-800 animate-pop-in">
                {selectedRoleObj?.icon}
                <span className="text-base font-semibold text-gray-700 dark:text-gray-200">{selectedRoleObj?.label}</span>
              </div>
              <span className="text-xs text-gray-400 dark:text-gray-500 mt-1">Login as {selectedRoleObj?.label}</span>
            </div>
            <div className="w-full h-px bg-gradient-to-r from-blue-200/60 via-indigo-200/60 to-transparent dark:from-blue-900/40 dark:via-indigo-900/40 dark:to-transparent mb-6" />
            ...
            <div className="space-y-4">
                ...
                <input
                  ...
                  placeholder="your.email@district.gov.gh"
                    disabled={false}
                />
              </div>
                {/* Password Field */}
